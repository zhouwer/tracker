...
FeatureTensor::FeatureTensor(const int maxBatchSize, const cv::Size imgShape, const int featureDim, int gpuID, ILogger* gLogger) 
    ...
    printf("inFeatureTensor::FeatureTensor... \n");
    cudaSetDevice(gpuID);
    ...
    initFlag = false;
    printf("ouFeatureTensor::FeatureTensor... \n");
}
...
bool FeatureTensor::getRectsFeature(const cv::Mat& img, DETECTIONS& det) {//now img  and  detetc img
	printf("inFeatureTensor::getRectsFeature... \n");
    ...
    for (auto& dbox : det) {
        ...
        std::cout  << "52---featu.c-------imgShape:" << imgShape<< "---shape-- " << std::endl;

        cv::resize(tempMat, tempMat, imgShape);
        mats.push_back(tempMat);   // kou de  mubiao
    }
    doInference(mats);
    // decode output to det
    stream2det(outputBuffer, det);  //row class
    printf("ouFeatureTensor::getRectsFeature... \n");
    return true;
...
void FeatureTensor::loadEngine(std::string enginePath) {
    // Deserialize model
    printf("inFeatureTensor::loadEngine... \n");
    ...
    printf("86... \n");
    initResource();
    printf("ouFeatureTensor::loadEngine... \n");
} 

void FeatureTensor::loadOnnx(std::string onnxPath) {
    printf("inFeatureTensor::loadOnnx... \n");
    ...
    assert(context != nullptr);
    initResource();
    printf("ouFeatureTensor::loadOnnx... \n");
}

int FeatureTensor::getResult(float*& buffer) {
    printf("inFeatureTensor::getResult... \n");
    if (buffer != nullptr)
        delete buffer;
    ...
    printf("ouFeatureTensor::getResult... \n");
    return curStreamSize;
    
}

void FeatureTensor::doInference(vector<cv::Mat>& imgMats) {
    printf("inFeatureTensor::doInference..vector<cv::Mat>& imgMats. \n");
    mat2stream(imgMats, inputBuffer);
    doInference(inputBuffer, outputBuffer);
    printf("ouFeatureTensor::doInference..vector<cv::Mat>& imgMats. \n");
}

void FeatureTensor::initResource() {
	printf("inFeatureTensor::initResource()... \n");
    ...
    initFlag = true;
    printf("ouFeatureTensor::initResource()... \n");
}

void FeatureTensor::doInference(float* inputBuffer, float* outputBuffer) {   
	printf("inFeatureTensor::doInference.float* inputBuffer, float* outputBuffer). \n");
    ...
    // cudaStreamSynchronize(cudaStream);
    printf("ouFeatureTensor::doInference.float* inputBuffer, float* outputBuffer). \n");
}

void FeatureTensor::mat2stream(vector<cv::Mat>& imgMats, float* stream) {
    
    printf("inFeatureTensor::mat2stream..111 \n");
    int imgArea = imgShape.area();
    std::cout  << "175-----featu.c---------imgArea:" << imgArea<< "danwei " << std::endl;


    curBatchSize = imgMats.size();
    std::cout  << "181-----featu.c---------curBatchSize:" << curBatchSize<< "danwei " << std::endl;
    ...
    printf("ouFeatureTensor::mat2stream..11 \n");
}

void FeatureTensor::stream2det(float* stream, DETECTIONS& det) {
    printf("inFeatureTensor:::stream2det..```` \n");
    int i = 0;
    ...
    printf("ouFeatureTensor:::stream2det..row class have feature`` \n");
}
