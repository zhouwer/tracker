...
{
	printf("in.and.out.instance = new linear_assignment();..-=--=. \n");
	
}

linear_assignment *linear_assignment::getInstance()
{
	printf("inlinear_assignment *linear_assignment::getInstance..---. \n");
        //std::cout <<"linear_..13..instance == NULL:" << *instance <<  std::endl;????????????????????/
    if(instance == NULL) instance = new linear_assignment();
    printf("oulinear_assignment *linear_assignment::getInstance...--- \n");
    return instance;
...
linear_assignment::matching_cascade(
        ...
        const DETECTIONS &detections, // have feature 256
        std::vector<int>& track_indices,   //confirmed
        std::vector<int> detection_indices)
//yolo bbox  and  quedingde  res
{
	printf("inlinear_assignment::matching_cascade..-=--=. \n");
    TRACHER_MATCHD res;
    ...
    for (auto i : tracks)
       std::cout << i.hits << ", ";
    std::cout << "]" << endl;
    std::cout << "detections.confidence :[" << std::endl;
    ...
    std::cout  << "62----yolobbox-detections.size:" << detections.size()<< "dd " << std::endl;
    for(size_t i = 0; i < detections.size(); i++) {
        detection_indices.push_back(int(i));
    }
    std::cout  << "66----detection_indices.size:" << detection_indices.size()<< "dd " << std::endl;
    std::vector<int> unmatched_detections;
    unmatched_detections.assign(detection_indices.begin(), detection_indices.end());
    res.matches.clear();
    std::vector<int> track_indices_l;
    std::cout  << "71---unmatched_detections.size():" << unmatched_detections.size()<< "dd " << std::endl;
    ...
    for(int level = 0; level < cascade_depth; level++) {
        ...
        //track_indices:confirmedtrack:[]
        for(int k:track_indices) {
            if(tracks[k].time_since_update == 1+level) //have many tracks   ???how
                track_indices_l.push_back(k);
        }
        //std::cout  << "81---track_indices_l.size():" << track_indices_l.size()<< "dd " << std::endl;
    ...
    std::cout  << "97----track_indices.size():" << track_indices.size()<< "dd " << std::endl;
    ...
    printf("oulinear_assignment::matching_cascade..=-===. \n");
    return res;
    
}

TRACHER_MATCHD
linear_assignment::min_cost_matching(tracker *distance_metric, // NN metric  (iou
        tracker::GATED_METRIC_FUNC distance_metric_func, // tracks dets _ind _ind ??  (iou
        float max_distance,
        std::vector<Track> &tracks,  //confirmed track
        const DETECTIONS &detections,
        std::vector<int> &track_indices,
        std::vector<int> &detection_indices) //detections, track_indices_l,unmatched_detections
{
	printf("inTRACHER_MATCHDlinear_assignment::min_cost_matching................NO.OUT?. \n");
    TRACHER_MATCHD res;
    ...
    if((detection_indices.size() == 0) || (track_indices.size() == 0)) {
        ...
        printf("ouTRACHER_MATCHDlinear_assignment::min_cost_matching.............135.......yes.OUT \n");
        return res;
    }
    ...
    printf("ouTRACHER_MATCHDlinear_assignment::min_cost_matching...............176.....yes.OUT \n");
    return res;
...
{
	printf("inDYNAMICMlinear_assignment::gate_cost_matrix... \n");
    // std::cout << "input cost matric" << cost_matrix << std::endl;
    int gating_dim = (only_position == true?2:4);
    double gating_threshold = KkalmanFilter::chi2inv95[gating_dim];
    ...
    printf("ouDYNAMICMlinear_assignment::gate_cost_matrix... \n");
    return cost_matrix;
